...
- `date`：决定文章网址（如 2026-09-10 → https://zhaohai.cc/2026/09/10/文章名/），写当天的日期。
- 标签规则：**每篇 ≤ 5 个，每个 ≤ 4 个汉字**。
- 写作完成后**删除模板里的使用说明注释块**（`<!-- ... -->`）。
...
- 图片建议规范（发布前处理）：
  - 宽边 ≤ 1200px；照片类转 JPG（体积小 10 倍）；体积尽量 ≤ 300KB
  - 文件名用英文/数字，如 `meng.jpg`、`chengdu-1.jpg`
...
| 忘了删模板注释块 | 发布前删除 `<!-- ... -->` |
| 标签超过 5 个 / 超 4 汉字 | 精简，≤5 个、每个 ≤4 汉字 |
| date 是模板占位值（2022-02-01 等） | 改成实际写作日期 |
| 图片太大（>1MB） | 压到 ≤300KB，照片转 JPG |
